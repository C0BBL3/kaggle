class Matrix():
    ...
    def rref(self, return_determinant=False):
        ...
        for i in range(0, copy_matrix.rows):  # i in range of the columns
            ...
            if pivot_row != None:
                ...
                if i != copy_matrix.rows - 1:  # if not last row
    ...
    def transpose(self, arr=None):
        if arr == None:
            return Matrix(elements=[[row[i] for row in self.elements] for i in range(self.cols)])
        else:
